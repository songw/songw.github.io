...
所谓条件概率，就是在事件 $B$ 发生的条件下，事件 $A$ 发生的概率，用 $P(A \mid B)$ 来表示。在下面的文氏图中，定义了事件 $A$ 和 $B$，以及他们的交集 $A\cap B$，$\overline A$ 为 $A$ 的补集。则在事件 $B$ 发生的条件下，事件 $A$ 发生的概率为 $P(A \mid B)$ = $\displaystyle{P (A \cap B)} \over \displaystyle{P(B)}$ => $P(A \cap B)=P(A|B)P(B)$；同理可得，$P(B|A)$ = $\displaystyle{P (A \cap B)} \over \displaystyle{P(A)} $ => $P(A \cap B)=P(B|A)P(A)$，于是 $P(A|B)P(B) = P(B|A)P(A)$。
